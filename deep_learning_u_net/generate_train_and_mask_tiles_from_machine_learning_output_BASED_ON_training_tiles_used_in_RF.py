...
factor=1
overlap = 16
PATCH_SIZE_LARGE, PATCH_SIZE_SMALL = 1024*factor, 64*factor
STEP_LARGE, STEP_SMALL = (PATCH_SIZE_LARGE-overlap)*factor, (PATCH_SIZE_SMALL-overlap)*factor
...
mask_arr_large = cv2.cvtColor(mask_arr_large_input, cv2.COLOR_BGR2GRAY)
n = []
for i in sorted(glob.glob(path_ml_img + 'train/masks/*.tif')): #Only to take the index of the masks. The files are not used as well
    name=i.split('/')[-1]
    name2=name.split('.')[0]
    n1=name2.split('_')[1]
    n2=name2.split('_')[2]
...
for i in tile_index:
    print(i)
    tile=mask_patches_large[i[0],i[1]]
    # plt.imshow(tile, cmap='jet')
    # cv2.imwrite(path_dst_masks + 'large/tile_' + str(i[0]) + '_' + str(i[1]) + '.tif', tile) #Saved only for confirmation purposes, deleted after sanity check was done
    patches = patchify(tile, (PATCH_SIZE_SMALL, PATCH_SIZE_SMALL), step=STEP_SMALL)[:, :, :, :]
    tiles_x = patches.shape[1]
    tiles_y = patches.shape[0]
    for j in range(0, tiles_y):
        for k in range(0, tiles_x):
            img = patches[j, k, :, :].astype(np.uint8)
            cv2.imwrite(path_dst_masks + 'tile_' + str(i[0]) + '_' + str(i[1]) + '_' + str(j) + '_' + str(k) + '.tif', img)
